...
# card$INGRESOS_GASTOS %>% select( variable, bin, points )
# cutpoints <- woe.tree.binning(data, "DEFAULT", "INGRESOS_GASTOS")
# woe.binning.plot(cutpoints)


# bins$EDAD
# card$EDAD %>% select( variable, bin, points )
# cutpoints <- woe.tree.binning(data, "DEFAULT", "EDAD")
# woe.binning.plot(cutpoints)

# bins$ANTIGUEDAD_LABORAL
# card$ANTIGUEDAD_LABORAL %>% select( variable, bin, points )
# cutpoints <- woe.tree.binning(data, "DEFAULT", "ANTIGUEDAD_LABORAL") # ds_Train_base
# woe.binning.plot(cutpoints)

# bins$MONTO_OPERACION
# cutpoints <- woe.tree.binning(data, "DEFAULT", "MONTO_OPERACION") # ds_Train_base
# woe.binning.plot(cutpoints)
# card$MONTO_OPERACION %>% select( variable, bin, points )

# bins$TOT_ACTIVOS
# cutpoints <- woe.tree.binning(data, "DEFAULT", "TOT_ACTIVOS") # ds_Train_base
# woe.binning.plot(cutpoints)

# bins$DEPARTAMENTO
# cutpoints <- woe.tree.binning(data, "DEFAULT", "DEPARTAMENTO") # ds_Train_base
# woe.binning.plot(cutpoints)

# bins$COD_PROFESION
# cutpoints <- woe.binning(data, "DEFAULT", "COD_PROFESION") # ds_Train_base
# woe.binning.plot(cutpoints)
# cutpoints <- woe.tree.binning(data, "DEFAULT", "COD_PROFESION") # ds_Train_base
# woe.binning.plot(cutpoints)
# as.character((cutpoints[[2]] %>% filter(Group.2 == "misc. level neg. + 304 + 200 + 351") %>% select(Group.1))$Group.1)
# as.character((cutpoints[[2]] %>% filter(Group.2 == "10 + 94 + 91 + 60 + 30 + 70 + 61 + 92 + 93 + 352 + 90") %>% select(Group.1))$Group.1)
# as.character((cutpoints[[2]] %>% filter(Group.2 == "50 + 319 + 324 + 95 + misc. level pos. + 321 + 170") %>% select(Group.1))$Group.1)
...
PDO <- 48 # 50 40
POINT0 <- 570 # 600 500
card <- scorecard(bins, model, points0 = POINT0, odds0 = 1/19, pdo = PDO) # odds0 = 1/19
train_score <- scorecard_ply(ds_Train, card, only_total_score=FALSE)
test_score <-  scorecard_ply(ds_Test, card, only_total_score=FALSE)
...
# save(model, file = "tarjetasModel.RData")
# save(cutoff, file = "tarjetasCutoff.RData")
# save(bins, file = "tarjetasBins.RData")
# save(card, file = "tarjetasCard.RData")
