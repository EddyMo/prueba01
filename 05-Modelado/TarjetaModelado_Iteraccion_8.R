...
breaks_list = list(
  ...
  # , TOT_ACTIVOS = c(1000, 7500, 15000, 17000) # c(3000,8000,16500,20000)
...
col_colinealidad <- c("PASIVO_ACTIVO", "TOT_PASIVOS"
  ...
  , "NIVEL_EDUCACION")
col_capacidad <- c("NIVEL_INGRESO", "CANTIDAD_HIJOS") # "TIPO_VIVIENDA", "TOT_PASIVOS", "ESTADO_CIVIL", "CARGO"
...
# str(dataTestBt)
# (dataTestBt %>% filter(Score > 600, Real == 1))$Operacion
# auxDsTest <- ds_Test %>% filter( OPERACION %in% (dataTestBt %>% filter(Score > 600, Real == 1))$Operacion )
...
# Prueba con el total
dataTotal <- read.csv("tarjetaDatosEtiquetadosConSaldos18meses90dias.csv", sep = "|", dec = ".", stringsAsFactors = FALSE)
dataTotal <- dataTotal %>%
  ...
  select( EDAD,ESTADO_CIVIL,ANTIGUEDAD_LABORAL,CARGO,TIPO_VIVIENDA,GENERO,MONTO_OPERACION,INGRESOS_GASTOS,COD_PROFESION,DEPARTAMENTO,TOT_ACTIVOS, DEFAULT, SALDO, OPERACION )
...
dataTotal_score <- dataTotal_score %>% 
  mutate( COD_PROFESION_points = ifelse( !is.na(COD_PROFESION_points), COD_PROFESION_points, 0 ), 
          CARGO_points = ifelse( !is.na(CARGO_points), CARGO_points, 0 ),
          ESTADO_CIVIL_points = ifelse( !is.na(ESTADO_CIVIL_points), ESTADO_CIVIL_points, 0 )
          ) %>% 
  mutate( score = ifelse ( !is.na(score), score , (card$basepoints$points + EDAD_points + ESTADO_CIVIL_points + ANTIGUEDAD_LABORAL_points + CARGO_points + TIPO_VIVIENDA_points + GENERO_points + MONTO_OPERACION_points + INGRESOS_GASTOS_points + COD_PROFESION_points + DEPARTAMENTO_points + TOT_ACTIVOS_points ) ) )
