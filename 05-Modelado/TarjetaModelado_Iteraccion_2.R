...
breaks_list = list(
  EDAD = c(27,31,35,37), # 27.27,30.93,35.55,36.7 | 29,33,4
  NIVEL_EDUCACION = c("1%,%3%,%7","2","4%,%5%,%6") #"1%,%3%,%7","2","4%,%5%,%6"
  ,INGRESOS_GASTOS = c(1.48,2.25,4.05)
  ,DEPARTAMENTO = c("SANTA CRUZ%,%ORURO", "LA PAZ", "COCHABAMBA%,%BENI%,%CHUQUISACA%,%PANDO%,%TARIJA")
  ,CARGO = c("1%,%2%,%10%,%12","5%,%9","3%,%4%,%6%,%7%,%8%,%11")
  ,ESTADO_CIVIL = c("1","2%,%4%,%5","3")
  ,PASIVO_ACTIVO = c(0.01)
  # ,COD_PROFESION = c("0%,%1%,%20%,%25%,%35%,%40%,%45%,%72%,%77%,%78%,%130%,%150%,%180%,%200%,%302%,%304%,%316%,%343%,%346%,%347%,%358%,%370%,%371%,%380%,%391%,%397%,%398%,%411%,%412%,%416%,%418%,%420%,%425%,%440%,%454%,%459%,%463%,%468%,%480%,%484%,%489%,%492%,%497%,%499%,%500%,%502%,%505%,%511%,%516%,%526%,%529",
  #                    "92%,%61%,%10",
  #                    "15%,%30%,%33%,%50%,%60%,%62%,%70%,%71%,%73%,%80%,%85%,%90%,%91%,%93%,%94%,%95%,%100%,%101%,%110%,%140%,%170%,%190%,%210%,%240%,%250%,%300%,%301%,%303%,%305%,%308%,%309%,%310%,%313%,%314%,%318%,%319%,%321%,%322%,%323%,%324%,%325%,%327%,%328%,%340%,%344%,%348%,%351%,%352%,%353%,%355%,%357%,%361%,%362%,%364%,%366%,%368%,%369%,%372%,%373%,%374%,%376%,%379%,%381%,%387%,%388%,%393%,%394%,%400%,%404%,%405%,%406%,%407%,%408%,%409%,%415%,%417%,%419%,%430%,%434%,%435%,%438%,%441%,%444%,%446%,%447%,%450%,%455%,%456%,%458%,%460%,%461%,%465%,%469%,%471%,%472%,%474%,%475%,%476%,%479%,%481%,%483%,%485%,%486%,%490%,%495%,%498%,%501%,%503%,%504%,%507%,%510%,%519%,%521")
  # ,TOT_PASIVOS = c(0.01)
  # ,TOT_ACTIVOS = c(8000)
  # ,TIPO_VIVIENDA = c("1%,%2","3","4")
  # ,ANTIGUEDAD_LABORAL = c(3)
  # ,NIVEL_INGRESO = c("NO INDICADO%,%MENOS DE $600", "$600 - $1200%,%$1201 - $2000%,%$2001 - $5000","MÁS DE $5000")
...
# bins$COD_PROFESION
# cutpoints <- woe.binning(ds_Train_base, "DEFAULT", "COD_PROFESION") # ds_Train_base
# woe.binning.plot(cutpoints)
# as.character( (cutpoints[[2]] %>% filter( Group.2 == "304 + 200 + misc. level neg." ))$Group.1 )
# as.character( (cutpoints[[2]] %>% filter( Group.2 == "misc. level pos." ))$Group.1 )


# bins$TIPO_VIVIENDA
# cutpoints <- woe.binning(data, "DEFAULT", "TIPO_VIVIENDA") # ds_Train_base
# woe.binning.plot(cutpoints)
...
# bins$DEPARTAMENTO
# cutpoints <- woe.binning(data, "DEFAULT", "DEPARTAMENTO") # ds_Train_base
# woe.binning.plot(cutpoints)
...
# bins$EDAD
# cutpoints <- woe.binning(data, "DEFAULT", "EDAD") # ds_Train_base
# woe.binning.plot(cutpoints)
# ds_Train_base %>% group_by(EDAD) %>% summarise( n()) %>% arrange(EDAD) %>% as.data.frame()
...
# bins$NIVEL_INGRESO
# cutpoints <- woe.binning(ds_Train_base, "DEFAULT", "NIVEL_INGRESO") # ds_Train_base
# woe.binning.plot(cutpoints)

# bins$MONTO_OPERACION
# cutpoints <- woe.binning(ds_Train_base, "DEFAULT", "MONTO_OPERACION") # ds_Train_base
# woe.binning.plot(cutpoints)
...
# bins$TOT_PASIVOS
# cutpoints <- woe.binning(data, "DEFAULT", "TOT_PASIVOS") # ds_Train_base
# woe.binning.plot(cutpoints)

# bins$PASIVO_ACTIVO
# cutpoints <- woe.binning(data, "DEFAULT", "PASIVO_ACTIVO") # ds_Train_base
# woe.binning.plot(cutpoints)
...
col_bajaRelevancia <- c("NRO_CODEUDORES") #,"CANTIDAD_HIJOS","TIPO_VIVIENDA"
