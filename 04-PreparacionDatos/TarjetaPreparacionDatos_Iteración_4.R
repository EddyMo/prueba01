...
listaBoxPlot[["MONTO_OPERACION"]] # outliers: 1378V, 29D ; > 20000 | < 1000
listaBoxPlot[["INGRESOS_GASTOS"]] # outliers: 590V, 12D ; > 7
listaBoxPlot[["CANTIDAD_HIJOS"]] # outliers: 8V ; > 4
listaBoxPlot[["TOT_ACTIVOS"]] # outliers: 1457V, 36D ; > 70000
listaBoxPlot[["TOT_PASIVOS"]] #  outliers: 134V, 3D ; > 30000
listaBoxPlot[["NRO_CODEUDORES"]] # nivel de discriminación poco significativo (solo 1 caso en Default con NRO_CODEUDORES = 2)
listaBoxPlot[["PASIVO_ACTIVO"]] # outliers: 8V ; > 1
listaBoxPlot[["ANTIGUEDAD_LABORAL"]] # outliers: 1107V, 66D ; > 10
...
# datos %>% filter( MONTO_OPERACION > 20000 | MONTO_OPERACION < 1000 ) %>% group_by(DEFAULT) %>% summarise( numOp = n() )
datos <- datos %>% filter( MONTO_OPERACION <= 20000 & MONTO_OPERACION >= 1000 )
...
#TOT_PASIVO - PASIVO_ACTIVO (0.79) ; TOT_ACTIVOS - TOT_PASIVOS (0.44) ; EDAD - TOT_ACTIVOS (0.19) ; EDAD - CANTIDAD_HIJOS (0.25) ; CANTIDAD_HIJOS - TOT_ACTIVOS (0.20) ; TOT_ACTIVOS - PASIVO_ACTIVO (0.19)
...
write.table(vistaMinable %>% select(-one_of(c("NRO_SOLICITUD","CODIGO_CLIENTE","NRO_PERSONA","NRO_MES_DEFAULT"))),
            file = "TarjetaVistaMinable_36meses90dias_Iteracion_4.csv", sep = ";", dec = ".", quote = FALSE, row.names = FALSE, na = "")
